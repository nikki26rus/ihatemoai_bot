...
BULLY_SYSTEM_PROMPT = """Ты дерзкий и саркастичный телеграм-бот в групповом чате между знакомыми.
На сообщения отвечай коротко: 1–2 предложения, максимум 200 символов.
Обязательно учитывай СМЫСЛ того, что написал человек — подкалывай именно по теме, а не абстрактно.
Стиль: ироничный, слегка хамский и дружески-саркастичный. Не унижай человека, не угрожай и не переходи на жёсткие оскорбления.
Мат допустим редко и только когда он естественно уместен; не делай его обязательным.
Если пользователь задал вопрос, прежде всего ответь по существу, а затем можешь добавить короткую колкость.
...
FALLBACK_QUESTION_RESPONSES = [
    "Интересный вопрос. Дай секунду, я мысленно закатываю глаза.",
    "Спросить не стыдно — стыдно было бы так и не узнать.",
    "Ладно, вопрос засчитан. Но гуглить ты тоже умеешь.",
    "Неплохая попытка переложить думание на бота.",
]

FALLBACK_GREETING_RESPONSES = [
    "О, привет. Чат переживёт и это.",
    "Здарова. Что на этот раз?",
    "Привет. Только не превращай это в совещание.",
    "О, ты. Ну заходи, раз уж пришёл.",
]

FALLBACK_COMPLAINT_RESPONSES = [
    "Неприятно. Но драматичный отчёт в чат немного лишний.",
    "Сочувствую, но звучит так, будто день решил с тобой поспорить.",
    "Вот это не повезло. Чат официально выразил своё «мда».",
    "Пожаловался — уже легче? Тогда продолжаем жить.",
]

FALLBACK_BRAG_RESPONSES = [
    "Неплохо. Запиши этот день в календарь, чтобы не забыть.",
    "Ладно, это действительно звучит достойно. Почти.",
    "Круто для тебя. Чат постарается выглядеть впечатлённым.",
    "Вау. Похвала выдаётся в порядке живой очереди.",
]

FALLBACK_GENERIC_RESPONSES = [
    "Смело. Не факт, что это было нужно, но смело.",
    "Прочитал. Осмысливаю степень необходимости этой информации.",
    "Это сообщение определённо существует.",
    "Спасибо, теперь в чате стало на одну мысль больше.",
    "Звучит убедительно, если не задавать лишних вопросов.",
]
    ...
    *,
    ...
    if replied_to_bot_text:
        ...
        user_message = (
            ...
            "Ответь по смыслу, с лёгкой саркастичной колкостью."
        )
    else:
